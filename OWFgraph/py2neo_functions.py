...
def add_property(label, name, property_key, property_value):
    print exe('MATCH (n:' + label.title() + '{name:\'' + name + '\'}) SET n.' + property_key + '=\'' + property_value +
              '\' return labels(n) as Label, n.name as Name, n.' + property_key + ' as ' + property_key.title())


# Delete relationship between two nodes. Requires source and target's label and name, and relationship type.
def del_relationship(label_source, name_source, relationship, label_target, name_target):
...
def add_relationship(label_source, name_source, relationship, label_target, name_target):
