...
class Trig(Component):
    ...
    def execute(self):
        self.f_x_sin = 0.5*sin(self.x)
        self.f_x_cos = 0.5*cos(self.x)
...
class Simulation(Assembly):
    ...
    def configure(self):
        ...
        self.trig_meta_model.surrogates['f_x_sin'] = FloatKrigingSurrogate()
...
if __name__ == "__main__":
    ...
    p.scatter(train_inputs, train_actual_sin, c='r', label="training data")
    p.scatter(inputs, predicted_sin, c='k', label="predicted result")
    p.legend()
    p.show()


    # p.scatter(train_inputs, train_actual_cos, c='r', label="training data")
    # p.scatter(inputs, predicted_cos, c='k', label="predicted result")
    # p.legend()
    # p.show()